        ...
        for objects in file.getElementsByTagName("objects"):
            ...
            for vehicle in objects.getElementsByTagName("vehicle"):
                ...
                if (type == "pedestrian"):
                    ...
                    print
                    "pedestrian:", anno.imageName, rect.x1, rect.y1, rect.x2, rect.y2
...
def saveXML(filename, annotations):
    ...
    if (ext == ".al"):
        ...
        print
        "invalid filename - .al(.gz|.bz2) is accepted"
        return
...
def getStats(annotations):
    ...
    for anno in annotations:
        ...
        for rect in anno.rects:
            ...
            if (rect.width() == 0):
                print
                "Warning: width=0 in image ", anno.imageName
                widths.append(1)
            else:
    ...
    "  Total # rects:", no
    print
    "     avg. Width:", avgWidth, " (", sqrt(varWidth), "standard deviation )"
    print
    "    avg. Height:", avgHeight, " (", sqrt(varHeight), "standard deviation )"
    print
    "     tiny rects:", noTiny, " (< 36 pixels)"
    print
    "    small rects:", noSmall, " (< 128 pixels)"
    print
...
def extractSubSet(gtIDL, detIDL):
    ...
    for i, gtAnno in enumerate(gtIDL):
        ...
        if (not imageFound):
            print
            "Image not found " + gtAnno.imageName + " !"
